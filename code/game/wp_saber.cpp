...
void G_Stagger(gentity_t* hit_ent, gentity_t* atk, qboolean allowAnyMove);
	...
	if (&g_entities[tr.entityNum])
	{
		...
		if (wp_saber_must_block(hit_ent, owner, qfalse, tr.endpos, -1, -1))
		{
			...
			if (wp_saber_must_block(&g_entities[tr.entityNum], owner, qfalse, tr.endpos, -1, -1))
			{
				...
				const qboolean other_holding_block = hit_ent->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING
					? qtrue
					: qfalse; //Normal Blocking
				const qboolean other_active_blocking = hit_ent->client->ps.ManualBlockingFlags & 1 << MBF_PROJBLOCKING
					? qtrue
					: qfalse; //Active Blocking
				const qboolean other_m_blocking = hit_ent->client->ps.ManualBlockingFlags & 1 << MBF_MBLOCKING
					? qtrue
					: qfalse; //Perfect Blocking

				if (hit_ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(hit_ent))
				{
					WP_SaberBlockNonRandom(hit_ent, tr.endpos, qfalse);
				}
				else
				{
					if (other_holding_block || other_active_blocking || other_m_blocking || hit_ent->client->ps.
		...
		else if (hit_ent->contents & CONTENTS_LIGHTSABER)
		{
			...
			if (attacker && attacker->client && attacker->client->ps.saberInFlight
				&& owner && owner->s.number == 0 && (g_saberAutoBlocking->integer && (owner->NPC && !
					G_ControlledByPlayer(owner)) || owner->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING ||
					owner->
...
void AnimateKnockaway(const gentity_t* self, gentity_t* inflictor, vec3_t impact)
{
	//do an knockaway.
...
qboolean WP_SaberMBlock(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean OtherHoldingBlock = victim->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING ? qtrue : qfalse;
	//Normal Blocking
	const qboolean OtherActiveBlocking =
		victim->client->ps.ManualBlockingFlags & 1 << MBF_PROJBLOCKING ? qtrue : qfalse; //Active Blocking
	const qboolean OtherMBlocking = victim->client->ps.ManualBlockingFlags & 1 << MBF_MBLOCKING ? qtrue : qfalse;
	//Perfect Blocking
	...
	if (g_in_cinematic_saber_anim(victim))
	{
		return qfalse;
	}
	...
	if (victim->s.number || OtherHoldingBlock || OtherActiveBlocking || OtherMBlocking || g_saberAutoBlocking->integer
		&& (victim->NPC && !G_ControlledByPlayer(victim)) || victim->client->ps.saberBlockingTime > level.time)
	{
			...
			&& !PM_SaberInKnockaway(victim->client->ps.saberMove))
		{
			...
			WP_SaberMBlockDirection(victim, saberHitLocation, qfalse);
		}

		victim->client->ps.saberEventFlags |= SEF_PARRIED;

		wp_saber_clear_damage_for_ent_num(attacker, victim->s.number, saberNum, bladeNum);
...
qboolean WP_SaberNPCMBlock(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean NPCBlocking = victim->client->ps.ManualBlockingFlags & 1 << MBF_NPCBLOCKING ? qtrue : qfalse;
	//Normal Blocking
	...
	if (victim->s.number || NPCBlocking || victim->client->ps.saberBlockingTime > level.time)
	{
		//either an NPC or a player who is blocking
		if (!PM_SaberInTransitionAny(victim->client->ps.saberMove)
			&& !PM_SaberInBounce(victim->client->ps.saberMove)
...
qboolean WP_SaberNPCParry(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean NPCBlocking = victim->client->ps.ManualBlockingFlags & 1 << MBF_NPCBLOCKING ? qtrue : qfalse;
	//Normal Blocking
...
qboolean WP_SaberBlockedBounceBlock(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean OtherHoldingBlock = victim->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING ? qtrue : qfalse;
	//Normal Blocking
	const qboolean OtherActiveBlocking =
		...
		&& (victim->NPC && !G_ControlledByPlayer(victim)) || victim->client->ps.saberBlockingTime > level.time)
	{
		...
		{
			//I'm not attacking, in transition or in a bounce or knockaway, so play a parry
			WP_SaberBouncedSaberDirection(victim, saberHitLocation, qfalse);
		}
...
qboolean WP_SaberFatiguedParry(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean OtherHoldingBlock = victim->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING ? qtrue : qfalse;
	//Normal Blocking
	const qboolean OtherActiveBlocking =
		...
		&& (victim->NPC && !G_ControlledByPlayer(victim)) || victim->client->ps.saberBlockingTime > level.time)
	{
		...
		{
			//I'm not attacking, in transition or in a bounce or knockaway, so play a parry
			WP_SaberFatiguedParryDirection(victim, saberHitLocation, qfalse);
		}
...
qboolean WP_SaberNPCFatiguedParry(gentity_t* victim, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean NPCBlocking = victim->client->ps.ManualBlockingFlags & 1 << MBF_NPCBLOCKING ? qtrue : qfalse;
	//Normal Blocking
...
qboolean WP_BrokenBoltBlockKnockBack(gentity_t* victim, gentity_t* attacker, qboolean allowAnyMove)
{
		...
		BLOCKPOINTS_TEN)
	{
		...
		G_Stagger(victim, attacker, qtrue);

		vec3_t throwDir = { 0, 0, 350 };

		WP_SaberDisarmed(victim, throwDir);
...
void G_StaggerAttacker(gentity_t* atk, gentity_t* hitEnt, qboolean allowAnyMove)
{
	...
	const int anim_choice = irand(0, 6);
	// this could possibly be based on animation done when the clash happens, but this should do for now.
	int use_anim;
...
void WP_SaberDamageTrace(gentity_t* ent, int saberNum, int bladeNum)
{
			...
			&& hitOwner->client->ps.saberLockTime < level.time)
		{
			...
			if (baseDamage)
			{
				...
				else if (entAttacking && hitOwnerDefending)
				{
					...
					{
						//an actual collision
						if (PM_SaberInnonblockableAttack(ent->client->ps.torsoAnim))
						{
							// a strong attack
							if (hitOwner->client->ps.blockPoints < BLOCKPOINTS_TEN)
							{
								//Low points = bad blocks
								if (saberNum == 0)
								{
									if (!(hitOwner->client->ps.saber[saberNum].saberFlags & SFL_NOT_DISARMABLE))
									{
										//knocked the saber right out of his hand!
										//Get a good velocity to send the saber in based on my parry move
										vec3_t throwDir;
										if (!PM_VelocityForBlockedMove(&ent->client->ps, throwDir))
										{
											PM_VelocityForSaberMove(&hitOwner->client->ps, throwDir);
										}
		...
		static_cast<qboolean>(saberNum == 0 && ent->client->ps.saberInFlight)))
	{
		...
		if (!saberInSpecial && (d_saberInfo->integer || g_DebugSaberCombat->integer))
		{
			if (g_saberRealisticCombat->integer == 3)
			{
...
void WP_SaberCatch(gentity_t* self, gentity_t* saber, qboolean switchToSaber)
{
	if (self->s.number < MAX_CLIENTS || G_ControlledByPlayer(self))
	{
		if (self->client->ps.forcePower > BLOCKPOINTS_FIVE && self->client->ps.blockPoints > BLOCKPOINTS_TWELVE && self
			->health > 0 && !PM_SaberInBrokenParry(self->client->ps.saberMove) && self->client->ps.saberBlocked !=
...
void WP_SaberGrab(gentity_t* self, gentity_t* saber)
{
	...
	if (self->s.number < MAX_CLIENTS || G_ControlledByPlayer(self))
	{
		if (self->client->buttons & BUTTON_BLOCK)
		{
			return;
		}
	}
...
void WP_SaberThrow(gentity_t* self, const usercmd_t* ucmd)
{
	...
	if (self->client->ps.weapon != WP_SABER)
	{
		...
		if (G_PointInBounds(saberent->currentOrigin, self->absmin, self->absmax))
		{
			...
			else
			{
				...
				{
					wp_block_points_regenerate_over_ride(self, BLOCKPOINTS_FATIGUE);
				}
			}
		}
	}
	...
	if (self->client->ps.saberEntityState == SES_LEAVING)
	{
		...
		if (self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2)
		{
			...
			else if (level.time - self->client->ps.saberKnockedTime >= 100)
			{
				if (WP_ForcePowerAvailable(self, FP_SABERTHROW, 10))
				{
					WP_ForcePowerDrain(self, FP_SABERTHROW, 1);
		...
		else
		{
			...
			else if (self->client->ps.forcePowerLevel[FP_SABERTHROW] == FORCE_LEVEL_1 && saberDist >= self->client->ps.
				saberEntityDist)
			{
			...
			else if (level.time - self->client->ps.saberThrowTime > 5000 || level.time - self->client->ps.
				saberKnockedTime > 5000 && !(ucmd->buttons & BUTTON_BLOCK))
			{
				//been out too long, or saber throw 1 went too far, return to me
				WP_SaberReturn(self, saberent);
			}
	...
	if (!(self->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING))
	{
		if (self->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(self))
		{
				...
				saberInFlight)
			{
				...
				if (self->client->ps.forcePower <= BLOCKPOINTS_HALF || self->client->ps.blockPoints <= BLOCKPOINTS_HALF)
				{
					switch (self->client->ps.forcePowerLevel[FP_SABER_DEFENSE])
					{
	...
	const int r_saber_num, const int r_blade_num)
{
	...
	if (BG_InKnockDown(self->client->ps.torsoAnim) || BG_InKnockDown(self->client->ps.legsAnim))
	{
		//can't block while knocked down or getting up from knockdown.
		return 0;
	}
		...
		if (!BG_SaberInNonIdleDamageMove(&atk->client->ps))
		{
			...
			if (self->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(self))
			{
				...
				if (self->client->ps.weapon == WP_SABER && self->client->ps.SaberActive() && !self->client->ps.
					saberInFlight)
				{
					if (self->client->ps.forcePower <= BLOCKPOINTS_HALF || self->client->ps.blockPoints <=
						BLOCKPOINTS_HALF)
					{
						switch (self->client->ps.forcePowerLevel[FP_SABER_DEFENSE])
						{
...
float wp_block_chance(const gentity_t* defender)
{
	...
	if (defender->s.number >= MAX_CLIENTS && defender->client->ps.weapon == WP_SABER)
	{
		//bots just randomly parry to make up for them not intelligently parrying.
...
qboolean WP_SaberBlockNonRandom(gentity_t* self, vec3_t hitloc, const qboolean missileBlock)
{
	...
	if (!inFront && self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_1)
	{
		...
		{
		case SS_STAFF:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		case SS_DUAL:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		default:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		}
	...
	else if (zdiff > 0)
	{
		if (rightdot > 0.3)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_TR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_TR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_TR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			}
		}
		else if (rightdot < -0.3)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_TL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_TL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_TL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				self->client->ps.weaponTime = Q_irand(300, 600);
				break;
			}
		}
		else
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_T_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_T_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_T_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
	...
	else
	{
		if (rightdot >= 0)
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_BR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_BR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_BR, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
		else
		{
			...
			{
			case SS_STAFF:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S7_BL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			case SS_DUAL:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S6_BL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			default:
				NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_BL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				break;
			}
			self->client->ps.weaponTime = Q_irand(300, 600);
		}
...
void wp_saber_update(gentity_t* self, const usercmd_t* ucmd)
{
		...
		else if (self->client->ps.saberBlocking == BLK_TIGHT || self->client->ps.saberBlocking == BLK_WIDE)
		{
			...
			if (!force_block
				&& (self->s.number && !Jedi_SaberBusy(self)
					...
					&& (self->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING || self->client->ps.saberBlockingTime >
						level.time))
	...
	if ((d_blockinfo->integer || g_DebugSaberCombat->integer) && !PM_SaberInAttack(self->client->ps.saberMove))
	{
...
{
	if (self->client->buttons & BUTTON_KICK
		&& (!(self->client->buttons & BUTTON_DASH))
		&& self->client->ps.pm_flags & PMF_KICK_HELD)
	{
		return qtrue;
	}
	return qfalse;
		...
		for (e = 0; e < numListedEntities; e++)
		{
			...
			if (render_impact)
			{
				if (tr.entityNum < ENTITYNUM_WORLD && trace_ent->takedamage)
				{
					...
					else if (npc_class == CLASS_OBJECT)
					{
						trace_ent->client->ps.powerups[PW_STUNNED] = level.time + 4000;
					}
					else
					{
						...
						else if (npc_class == CLASS_BOBAFETT ||
							npc_class == CLASS_MANDO)
						{
							...
							{
								G_Stagger(trace_ent, self, qtrue);
							}
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								G_Damage(trace_ent, self, self, dir, tr.endpos, damagelow, DAMAGE_DEATH_KNOCKBACK,
									MOD_LIGHTNING_STRIKE, hitLoc);
								if (trace_ent && trace_ent->health <= 50)
								{
									//turn it off?
									trace_ent->client->ps.SaberDeactivate();
									G_SoundOnEnt(trace_ent, CHAN_WEAPON, "sound/weapons/saber/saberoffquick.mp3");
...
//	int fp_block_cost;
//	constexpr qboolean saber_light_block = qtrue;
//
...
//	if (defender->client && defender->NPC)
//	{
...
//	}
//
//	if (defender->client->ps.forcePower < 10)
//	{
//		return qfalse;
//	}
//
...
//		if (defender->client->ps.blockPoints < BLOCKPOINTS_HALF)
//		{
//			WP_ForcePowerDrain(defender, FP_ABSORB, fp_block_cost);
//		}
//		defender->client->ps.blockPoints--;
//	}
//	return qtrue;
//}
	...
	if (trace_ent && trace_ent->takedamage)
	{
			...
			trace_ent->enemy == self)
		{
			...
			if (trace_ent->client && trace_ent->health > 0)
			{
				...
				else if (trace_ent->s.weapon == WP_SABER && trace_ent->client->ps.SaberActive())
				{
						...
						&& trace_ent->client->ps.blockPoints > 5)
					{
						...
						trace_ent->client->ps.blockPoints--;
					}
					else if (in_camera
						&& trace_ent->NPC
						&& trace_ent->client->ps.SaberActive()
			...
			if (trace_ent->client)
			{
				...
				if (trace_ent->health <= 0 || (npc_class == CLASS_ATST || npc_class == CLASS_GONK ||
					npc_class == CLASS_INTERROGATOR || npc_class == CLASS_MARK1 ||
					npc_class == CLASS_MARK2 || npc_class == CLASS_MOUSE ||
					npc_class == CLASS_PROBE || npc_class == CLASS_PROTOCOL ||
					npc_class == CLASS_R2D2 || npc_class == CLASS_R5D2 ||
