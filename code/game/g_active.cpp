...
void ClientTimerActions(gentity_t* ent, int msec)
{
	...
	while (client->timeResidual >= 1000)
	{
		...
		if (ent->client->ps.saberAttackChainCount > MISHAPLEVEL_NONE && (ent->s.number < MAX_CLIENTS ||
...
{
	const qboolean ActiveBlocking = ent->client->ps.ManualBlockingFlags & 1 << MBF_PROJBLOCKING ? qtrue : qfalse;
	//manual Blocking
...
void ClientThink_real(gentity_t* ent, usercmd_t* ucmd)
{
	...
	if (ent->client->ps.weapon == WP_MELEE && (ent->client->NPC_class == CLASS_BOBAFETT || ent->client->NPC_class ==
		CLASS_MANDO))
	{
