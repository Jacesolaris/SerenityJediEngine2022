...
static void PM_Footsteps(void)
{
	...
	qboolean HoldingBlock = pm->ps->ManualBlockingFlags & 1 << MBF_BLOCKING ? qtrue : qfalse;
...
int PM_ReadyPoseForSaberAnimLevelPlayer(void)
{
	...
	else
	{
		...
		else
		{
			...
			{
				//walking forward
				if (pm->cmd.buttons & BUTTON_WALKING && !HoldingBlock &&
					...
					pm->cmd.rightmove < 0)
				{
					...
					if (pm->ps->saberAnimLevel == SS_STAFF)
					{
						if (pm->ps->SaberStaff() && (!pm->ps->saber[0].singleBladeStyle || pm->ps->saber[0].blade[
							1].active)) //saber staff with more than first blade active
						{
							return BOTH_WALK_STAFF;
						}
							return BOTH_WALK1;
					}
					if (pm->ps->saberAnimLevel == SS_TAVION)
					{
						return BOTH_WALK1;
					}
	...
	if (!PM_RidingVehicle())
	{
		if (!manualBlocking)
		{
			...
			else if (PM_SpinningSaberAnim(pm->ps->legsAnim))
			{
				...
				PMF_DUCKED) || HoldingBlock && pm->cmd.buttons & BUTTON_WALKING)
			{
		...
		if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive())
		{
			if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)))
			{
				if (HoldingBlock)
				{
					...
					{
						PM_SaberFatigue(pm->ps, new_move, anim); //drainblockpoints highcost
					}
				}
			}
			else
				...
				if (pm->gent && pm->gent->client)
				{
					...
					else
					{
						...
						if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())
						{
							...
							else if (pm->cmd.buttons & BUTTON_KICK && (!(pm->cmd.buttons & BUTTON_DASH))
								&& pm->ps->communicatingflags & 1 << KICKING)
							{
								PM_MeleeKickForConditions();
							}
