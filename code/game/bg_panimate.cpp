	...
	const int fatigued)
{
	...
	if (anim >= BOTH_A1_T__B_ && anim <= BOTH_ROLL_STAB && !pm_saber_in_special_attack(anim)
	...
	int* legsAnimTimer, gentity_t* gent, int blendTime) // default blendTime=350
{
	...
	const qboolean ActiveBlocking = gent->client->ps.ManualBlockingFlags & 1 << MBF_PROJBLOCKING ? qtrue : qfalse;
	...
	if (!PM_WalkingOrRunningAnim(anim) && !PM_InKataAnim(anim))
	{
		//make smooth animations
		if ((ActiveBlocking || HoldingBlock) && anim == LS_READY)
		{
			blendTime *= 1.8f;
		}
	}
...
void PM_TorsoAnimLightsaber()
{
	...
	{
		//ready
		if (pm->ps->weapon == WP_SABER && pm->ps->SaberLength() && (pm->ps->SaberActive() || !g_noIgniteTwirl->
			integer))
		{
			//saber is on
