	...
	const float size_time_scale)
{
	...
	{
		//burn mark with glow
		int lifeTime = (1.01 - static_cast<float>(hit_ent->health) / hit_ent->max_health) * static_cast<float>(
			Q_irand(5000, 10000));
		float size;
					...
					if (!noMarks)
					{
						...
						if (client->ps.saber[saberNum].blade[bladeNum].trail.haveOldPos[i])
						{
							...
							else if (!i)
							{
								...
								VectorSubtract(client->ps.saber[saberNum].blade[bladeNum].trail.oldPos[i], trace.endpos,
									uaxis);
