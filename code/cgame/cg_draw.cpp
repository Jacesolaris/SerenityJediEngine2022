...
static void CG_DrawHUD(const centity_t* cent)
{
		...
		G_IsRidingVehicle(cent->gent))
	{
		...
		if (cent->currentState.weapon == WP_SABER)
		{
			constexpr int y = 415;
			constexpr int x = 26;
			CG_DrawblockPoints(x, y, hudRatio);
		}
	}
		...
		G_IsRidingVehicle(cent->gent))
	{
		...
		if (cent->currentState.weapon == WP_SABER)
		{
			...
			CG_Drawsaberfatigue(cent, sectionXPos, sectionYPos, hudRatio);
			CG_DrawblockingMode(cent, sectionXPos, sectionYPos, hudRatio);
		}
		else if (cent->currentState.weapon == WP_SABER && !cent->gent->client->ps.SaberActive())
		{
			CG_DrawForcePower(cent, sectionXPos, sectionYPos, hudRatio);
			CG_Drawgunfatigue(cent, sectionXPos, sectionYPos, hudRatio);
		...
		else if (cent->currentState.weapon == WP_MELEE)
		{
			CG_DrawForcePower(cent, sectionXPos, sectionYPos, hudRatio);
			CG_Drawgunfatigue(cent, sectionXPos, sectionYPos, hudRatio);
			CG_Drawweapontype(cent, sectionXPos, sectionYPos, hudRatio);
		}
		else
		{
			CG_Drawgunfatigue(cent, sectionXPos, sectionYPos, hudRatio);
...
constexpr auto BLOCKPOINT_BAR_RANGE = 125;
