...
//---------------------------------------------------------
{
	...
	while (traces < 10)
	{
		...
		if (G_DoDodge(traceEnt, ent, tr.endpos, -1, &damage, MOD_DISRUPTOR))
		{
...
//---------------------------------------------------------
{
	...
	for (int i = 0; i < traces; i++)
	{
		...
		if (d_projectileGhoul2Collision.integer)
		{
			trap->Trace(&tr, start, shotMins, shotMaxs, end, skip, MASK_SHOT, qfalse,
				G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_THICK | G2TRFLAG_HITCORPSES,
				g_g2TraceLod.integer);
		}
		else
...
void FireWeapon(gentity_t* ent, qboolean altFire)
{
	...
	if (!DoesnotDrainMishap(ent))
	{
		...
		else
		{
			...
			{
				G_AddMercBalance(ent, Q_irand(BLASTERMISHAPLEVEL_MININACCURACY, BLASTERMISHAPLEVEL_RUNINACCURACY));
				// 1 was not enough
			}
		}
	}
