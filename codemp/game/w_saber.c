...
extern void SabBeh_SaberShouldBeDisarmedBlocker(gentity_t* blocker, const gentity_t* attacker);
...
int wp_saber_must_m_block(gentity_t* self, const gentity_t* atk, qboolean check_b_box_block, vec3_t point, int r_saber_num,
	int r_blade_num);
extern void SabBeh_AddMishap_Attacker(gentity_t* attacker, gentity_t* blocker);
extern qboolean PM_WalkingOrRunningAnim(int anim);
extern void SabBeh_AnimateHeavySlowBounceAttacker(gentity_t* attacker, gentity_t* Blocker);
...
void sab_beh_run_saber_behavior(gentity_t* attacker, gentity_t* blocker, const int saber_num, const int blade_num,
	vec3_t hit_loc,
	qboolean* did_hit, const qboolean other_hit_saber_blade)
{
	if (!blocker)
	{
	...
	if (BG_SaberInNonIdleDamageMove(&attacker->client->ps, attacker->localAnimIndex)) //Attacker is attacking
	{
		//Attacker is attacking
		if (BG_SaberInNonIdleDamageMove(&blocker->client->ps, blocker->localAnimIndex)) //and Blocker is attacking
		{
			//and Blocker is attacking
			SabBeh_AttackVsAttack(attacker, blocker); //both attacking
			if (d_combatinfo.integer || g_DebugSaberCombat.integer && !(attacker->r.svFlags & SVF_BOT))
			{
				Com_Printf(S_COLOR_ORANGE"SabBeh_AttackVsAttack\n");
			}
		}
		else if (wp_saber_must_block(blocker, attacker, qfalse, hit_loc, -1, -1) || wp_saber_must_m_block(
			blocker, attacker, qfalse, hit_loc, -1, -1))
		{
			//and Blocker is blocking or parrying
			SabBeh_AttackvBlock(attacker, blocker, saber_num, blade_num, hit_loc, other_hit_saber_blade);
			*did_hit = qfalse;
			if (d_combatinfo.integer || g_DebugSaberCombat.integer && !(attacker->r.svFlags & SVF_BOT))
			{
				Com_Printf(S_COLOR_ORANGE"Attacker SabBeh_AttackvBlock\n");
			}
		}
		else
		{
	...
	else if (wp_saber_must_block(attacker, blocker, qfalse, hit_loc, -1, -1) || wp_saber_must_m_block(
		attacker, blocker, qfalse, hit_loc, -1, -1))
	{
		//Attacker is blocking or parrying
		if (BG_SaberInNonIdleDamageMove(&blocker->client->ps, blocker->localAnimIndex)) //and Blocker is attacking
		{
			//and Blocker is attacking
			SabBeh_AttackvBlock(blocker, attacker, saber_num, blade_num, hit_loc, qtrue);
			if (d_combatinfo.integer || g_DebugSaberCombat.integer && !(blocker->r.svFlags & SVF_BOT))
			{
				Com_Printf(S_COLOR_ORANGE"Blocker SabBeh_AttackvBlock\n");
			}
		}
		else if (wp_saber_must_block(blocker, attacker, qfalse, hit_loc, -1, -1) || wp_saber_must_m_block(
			blocker, attacker, qfalse, hit_loc, -1, -1))
...
void SaberUpdateSelf(gentity_t* ent)
{
	...
	else
	{
		...
		//#ifdef DEBUG_SABER_BOX
		if ((d_blockinfo.integer || g_DebugSaberCombat.integer) && !PM_SaberInAttack(
			g_entities[ent->r.ownerNum].client->ps.saberMove))
		{
			vec3_t dbg_mins;
			vec3_t dbg_maxs;
...
int wp_saber_must_m_block(gentity_t* self, const gentity_t* atk, const qboolean check_b_box_block, vec3_t point,
...
qboolean G_DoDodge(gentity_t* self, gentity_t* shooter, vec3_t dmgOrigin, int hitLoc, int* dmg, int mod)
{
	...
	//saved copy of the original damage level.  This is used determine the kickback power for slash damage dodges.
	...
	vec3_t saber_start, vec3_t saber_end, const int tr_mask)
{
	...
	qboolean hit_saber_blade = qfalse;
		...
		level.time) && !(self->r.svFlags & SVF_BOT))
	{
		...
		//Play-testing has indicated that using attack damage for transition moves results in unfair insta-hits.
		dmg = SABER_NO_DAMAGE;
		idle_damage = qtrue;
	}
	else if (PM_KickMove(self->client->ps.saberMove)
		...
		g_entities[tr.entityNum].inuse)
	{
		...
		if (wp_saber_must_block(blocker, self, qfalse, tr.endpos, -1, -1)) //Holding Block Button only
		{
			...
			{
				Com_Printf(S_COLOR_CYAN"Controlled Blocking starts here 1\n");
			}

			if (!PM_SaberInnonblockableAttack(self->client->ps.torsoAnim))
			{
				if (wp_saber_must_m_block(blocker, self, qfalse, tr.endpos, -1, -1))
					//Holding Block Button + attack button
				{
					if (blocker->client->ps.ManualBlockingFlags & 1 << MBF_MBLOCKING) //Perfect blocking
					{
						WP_SaberMBlockDirection(blocker, tr.endpos, qfalse);
						...
						if (self->r.svFlags & SVF_BOT)
						{
							SabBeh_AddMishap_Attacker(self, blocker);
						}
						else
						{
							SabBeh_AnimateHeavySlowBounceAttacker(self, blocker);
						}
					}
					else if (blocker->client->ps.ManualBlockingFlags & 1 << MBF_NPCBLOCKING) //Handles NPC blocking
					{
						if (blocker->client->ps.fd.blockPoints <= BLOCKPOINTS_HALF)
						{
							if (blocker->client->ps.fd.blockPoints <= BLOCKPOINTS_FOURTY)
							{
								WP_SaberFatiguedParryDirection(blocker, tr.endpos, qfalse);
							}
							else
							{
								WP_SaberBlockNonRandom(blocker, tr.endpos, qfalse);
							}
					...
					else
					{
						...
						if (blocker->client->ps.fd.blockPoints <= BLOCKPOINTS_FATIGUE)
						{
							...
							{
								SabBeh_SaberShouldBeDisarmedBlocker(blocker, self);
					...
					ManualBlockingFlags & 1 << MBF_PROJBLOCKING)) //Holding block button only (spamming block)
				{
					...
					{
						WP_SaberBouncedSaberDirection(blocker, tr.endpos, qfalse);
		...
		g_entities[tr.entityNum].inuse)
	{
		...
		else if (real_trace_result == REALTRACE_SABERBLOCKHIT)
		{
			...
			if (wp_saber_must_block(blocker, self, qfalse, tr.endpos, -1, -1)) //Holding Block Button only
			{
				//hit victim is able to block, block!
				did_hit = qfalse;
				hit_saber_blade = qtrue;

				if ((d_blockinfo.integer || g_DebugSaberCombat.integer) && !(blocker->r.svFlags & SVF_BOT))
				{
					Com_Printf(S_COLOR_CYAN"Bot Blocking starts here 1\n");
				}
		...
		else if (real_trace_result == REALTRACE_HIT)
		{
			//successfully hit another player's saber blade directly
			hit_saber_blade = qtrue;
			if (wp_saber_must_block(blocker, self, qfalse, tr.endpos, -1, -1) && !(blocker->r.svFlags & SVF_BOT))
				//Holding Block Button only
			{
				//hit victim is able to block, block!
				did_hit = qfalse;
				...
				if ((d_blockinfo.integer || g_DebugSaberCombat.integer) && !(blocker->r.svFlags & SVF_BOT))
				{
					Com_Printf(S_COLOR_CYAN"realTraceResult Blocking starts here 1\n");
				}
	...
	sab_beh_run_saber_behavior(self, blocker, r_saber_num, r_blade_num, tr.endpos, &did_hit, hit_saber_blade);
		...
		if (veclen < dist)
		{
			...
			if (tr.fraction == 1 || tr.entityNum == ent->s.number)
			{
				...
				if (!saberOwner->client->ps.isJediMaster && WP_SaberCanBlock(ent, tr.endpos, 0, MOD_SABER, qfalse, 999))
				{
					...
					{
						if (OtherHoldingBlock || OtherActiveBlocking || OtherMBlocking || ent->client->ps.
...
float wp_block_chance(const gentity_t* defender)
{
	...
	if ((defender->r.svFlags & SVF_BOT || defender->s.eType == ET_NPC) && defender->client->ps.weapon == WP_SABER)
	{
		//bots just randomly parry to make up for them not intelligently parrying.
...
qboolean WP_SaberBlockNonRandom(gentity_t* self, vec3_t hitloc, const qboolean missileBlock)
{
	...
	if (!inFront && self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_1)
	{
		...
		{
		case SS_STAFF:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P7_S1_B1_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		case SS_DUAL:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P6_S1_B1_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		default:
			NPC_SetAnim(self, SETANIM_TORSO, BOTH_P1_S1_B1_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			break;
		}
...
qboolean WP_SaberCanBlock(gentity_t* self, vec3_t point, int dflags, int mod, const qboolean projectile, int attackStr)
{
	...
	if (self->client->ps.ManualblockStartTime >= level.time)
	{
		return qfalse;
	}
...
qboolean WP_SaberMBlock(gentity_t* blocker, gentity_t* attacker, int saberNum, int bladeNum)
{
	const qboolean OtherHoldingBlock = blocker->client->ps.ManualBlockingFlags & 1 << MBF_BLOCKING ? qtrue : qfalse;
	//Normal Blocking
	const qboolean OtherActiveBlocking = blocker->client->ps.ManualBlockingFlags & 1 << MBF_PROJBLOCKING
		? qtrue
		: qfalse; //Active Blocking
	const qboolean OtherMBlocking = blocker->client->ps.ManualBlockingFlags & 1 << MBF_MBLOCKING ? qtrue : qfalse;
	//Perfect Blocking
	...
	if (blocker->s.number || OtherHoldingBlock || OtherActiveBlocking || OtherMBlocking || blocker->client->ps.
		saberBlockingTime > level.time)
	{
...
qboolean WP_SaberNPCMBlock(gentity_t* blocker, gentity_t* attacker, int saberNum, int bladeNum)
{
	...
	if (blocker->s.number || NPCBlocking || blocker->client->ps.saberBlockingTime > level.time)
	{
		//either an NPC or a player who is blocking
		if (!PM_SaberInTransitionAny(blocker->client->ps.saberMove)
			&& !PM_SaberInBounce(blocker->client->ps.saberMove)
