	...
	while (1)
	{
		...
		if (fps < 0)
		{
...
#ifdef __SABER_ANIMATION_SMOOTH__
			{
				...
				{
					if (saberMoveData[x].animToUse + 77 == anim_num) // SS_MEDIUM
					{
						animset[anim_num].frameLerp *= 1.1;
						break;
					}
#ifdef __NO_SABER_SPINAROUND__
					if (saberMoveData[x].animToUse + 77 * 2 == anim_num) // // SS_STRONG
					{
						animset[anim_num].frameLerp *= 1.2;
						break;
					}
					if (saberMoveData[x].animToUse + 77 * 3 == anim_num) // SS_DESANN
					{
						animset[anim_num].frameLerp *= 1.2;
						break;
					}
...
float bg_get_torso_anim_point(const playerState_t* ps, const int AnimIndex)
{
