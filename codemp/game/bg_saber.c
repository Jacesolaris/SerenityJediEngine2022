...
qboolean PM_SaberBlocking(void)
{
	...
	if (pm->ps->saberBlocked)
	{
		...
		case BLOCKED_ATK_BOUNCE:
			...
			{
				if (pm->ps->userInt3 & 1 << FLAG_SLOWBOUNCE && !(pm->ps->userInt3 & 1 << FLAG_OLDSLOWBOUNCE)
		...
		case BLOCKED_BACK:
			if (pm->ps->fd.saberAnimLevel == SS_DUAL)
			{
				PM_SetAnim(SETANIM_TORSO, BOTH_P6_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
			else if (pm->ps->fd.saberAnimLevel == SS_STAFF)
			{
				PM_SetAnim(SETANIM_TORSO, BOTH_P7_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
			else
			{
				PM_SetAnim(SETANIM_TORSO, BOTH_P1_S1_B_, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
			break;
		default:
			pm->ps->saberBlocked = BLOCKED_NONE;
...
void PM_WeaponLightsaber(void)
{
	...
	if (pm->cmd.buttons & BUTTON_ALT_ATTACK)
	{
		...
		if (pm->ps->saberInFlight && pm->cmd.buttons & BUTTON_KICK && !(pm->cmd.buttons & BUTTON_DASH))
		{
			//kick after doing a saberthrow,whalst saber is still being controlled
			if (!(pm->cmd.buttons & BUTTON_ATTACK)
				&& pm->ps->communicatingflags & 1 << KICKING) //not trying to swing the saber
			{
		...
		else if (!PM_SaberThrowable() && pm->cmd.buttons & BUTTON_ALT_ATTACK && !(pm->cmd.buttons & BUTTON_DASH)) //not trying to swing the saber
		{
		...
		else if (!PM_SaberThrowable() && pm->cmd.buttons & BUTTON_KICK && !(pm->cmd.buttons & BUTTON_DASH)) //not trying to swing the saber
		{
			//kick instead of doing a throw
			if (PM_DoSlap())
			{
...
weapChecks:
	...
	if (pm->ps->saberInFlight && pm->ps->forceHandExtend != HANDEXTEND_SABERPULL
		&& pm->cmd.buttons & BUTTON_ALT_ATTACK && !(pm->cmd.buttons & BUTTON_DASH))
	{
	...
	if (pm->ps->saberInFlight && pm->ps->forceHandExtend != HANDEXTEND_SABERPULL
		&& pm->cmd.buttons & BUTTON_KICK && !(pm->cmd.buttons & BUTTON_DASH))
	{
		//don't have our saber so we can punch instead.
		PM_DoSlap();
		return;
