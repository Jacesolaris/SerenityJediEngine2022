...
==================
*/
int BotAI_GetClientState(const int client_num, playerState_t* state)
{
...
void BotUpdateInput(bot_state_t* bs, const int time, const int elapsed_time)
{
	...
	if (bs->cur_ps.weapon == WP_SABER)
	{
		if (bs->currentEnemy
			...
			&& (VectorDistance(g_entities[bs->cur_ps.clientNum].r.currentOrigin, bs->currentEnemy->r.currentOrigin) <
				300 || walktime[bs->cur_ps.clientNum] > level.time))
		{
...
void SaberCombatHandling(bot_state_t* bs)
{
		...
		en_down == mid_down)
	{
		...
		if (bs->currentEnemy && bs->currentEnemy->client)
		{
			if (!PM_SaberInSpecial(bs->currentEnemy->client->ps.saberMove) && bs->frame_Enemy_Len > 90 && bs->
				saberBFTime > level.time && bs->saberBTime > level.time && bs->beStill < level.time && bs->saberSTime <
				level.time)
			{
				bs->beStill = level.time + Q_irand(500, 1000);
				bs->saberSTime = level.time + Q_irand(1200, 1800);
			}
			else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80.0f && (Q_irand(1, 10)
				< 8 && bs->saberBFTime < level.time || bs->saberBTime > level.time ||
				PM_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove ==
				LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_GRIEV || bs->currentEnemy->
				client->
				ps.saberMove == LS_SPINATTACK_DUAL))
			{
				vec3_t vs;
				vec3_t groundcheck;
				int idealDist;
				...
				if (PM_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove ==
					LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL)
				{
					idealDist = 256;
				}
				else
...
#endif
	{
		...
		if (bs->cur_ps.fd.forcePowersKnown & 1 << FP_PUSH && (bs->doForcePush > level.time || bs->cur_ps.fd.
			forceGripBeingGripped > level.time) && level.clients[bs->client].ps.fd.forcePower > forcePowerNeeded[
				level.
					clients[bs->client].ps.fd.forcePowerLevel[FP_PUSH]][FP_PUSH]
