	...
	do
	{
		...
		for (const searchpath_t* search = fs_searchpaths; search; search = search->next) {
			...
			if (search->pack && search->pack->hashTable[hash]) {
				...
				// autoexec_mp.cfg and SerenityJediEngine2022-SP.cfg can only be loaded outside of pk3 files.
				if (isUserConfig)
				{
					continue;
				}
					...
					if (!FS_FilenameCompare(pakFile->name, filename)) {
						...
						if (!(pak->referenced & FS_GENERAL_REF)) {
							if (!FS_IsExt(filename, ".shader", l) &&
								...
								Q_stricmp(filename, "SerenityJediEngine2022-jampgamex86.dll") != 0 &&
...
void FS_Startup(const char* gameName) {
	...
	fs_gamedirvar = Cvar_Get("fs_game", "SerenityJediEngine2022", CVAR_INIT | CVAR_SYSTEMINFO, "Mod directory");
